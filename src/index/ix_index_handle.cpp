 ...
 * @param transaction 事务参数，如果不需要则默认传入nullptrsu
 * @return 返回目标叶子结点
...
IxNodeHandle *IxIndexHandle::FindLeafPage(const char *key, Operation operation, Transaction *transaction) {
    ...
    page_id_t nowNode;
    IxNodeHandle *nodeHandle = FetchNode(getRootPageNo()); 

    // 2. 从根节点开始不断向下查找目标key
    // 从根沿着树一直向下找到叶子节点，并且每个找过的节点都要pin住
    while (!nodeHandle->IsLeafPage()) {
        // 定位查找的key所在的孩子节点
        nowNode = nodeHandle->InternalLookup(key);
        nodeHandle = FetchNode(nowNode);
    }
...
void IxIndexHandle::InsertIntoParent(IxNodeHandle *old_node, const char *key, IxNodeHandle *new_node,
    ...
    if (old_node->IsRootPage()) {
     ...
     IxNodeHandle *newRoot = CreateNode();
     ...
     // 3. 获取key对应的rid，并将(key, rid)插入到父亲结点
     // 将new_node的第一个key插入到父结点，其位置在 父结点指向old_node的孩子指针 之后
     parent->Insert(key, Rid{new_node->GetPageNo(), -1});
     // 4. 如果父亲结点仍需要继续分裂，则进行递归插入
...
bool IxIndexHandle::delete_entry(const char *key, Transaction *transaction) {
	...
	if(leaf->Remove(key) == size){// cannot remove not found
		buffer_pool_manager_->UnpinPage(leaf->GetPageId(), false);	
		return false;
	}
	else{
...
bool IxIndexHandle::CoalesceOrRedistribute(IxNodeHandle *node, Transaction *transaction) {
   ...
   if(node->IsRootPage()) //judge root
	...
	if(node->GetSize() >= node->GetMinSize()) { //>= half full, no need to coalesce
	...
	if(pos){
		brother_node = FetchNode(node->GetPrevLeaf());//must have left bro
	}
	else{
		brother_node = FetchNode(node->GetNextLeaf());
	}
	//unpin page
	if(node->GetSize() + brother_node->GetSize() >= node->GetMinSize() * 2){//browor st two both half full
...
void IxIndexHandle::Redistribute(IxNodeHandle *neighbor_node, IxNodeHandle *node, IxNodeHandle *parent, int index) {
    ...
    // 注意：neighbor_node的位置不同，需要移动的键值对不同，需要分类讨论
     if(index != 0){ //neighbor is left
		int pos = neighbor_node->GetSize() -1;//neighbor 's rightest
