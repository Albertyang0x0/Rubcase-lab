...
int IxNodeHandle::lower_bound(const char *target) const {
    ...
    int op=1;
        while (left < right) {
            mid = (left + right) / 2;
            int compareRes = ix_compare(target, get_key(mid), file_hdr->col_type, file_hdr->col_len);
            if (compareRes >= 0) {
                if (op == 1 && compareRes == 0) {
                    right = mid;
                } else {
                    left = mid + 1;
                }
...
page_id_t IxNodeHandle::InternalLookup(const char *key) {
    ...
    // 3. 返回页面编号
    int index = upper_bound(key); //内部结点每个key右边的value指向的孩子结点中的键均大于等于该key，每个key左边的value指向的孩子结点中的键均小于该key
...
void IxNodeHandle::insert_pairs(int pos, const char *key, const Rid *rid, int n) {
    ...
    memmove(pos_key + n * file_hdr->col_len, pos_key, resSize * file_hdr->col_len);
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    memmove(pos_rid + n, pos_rid, resSize * sizeof(Rid));

   
    memcpy(pos_key, key, n * file_hdr->col_len);
